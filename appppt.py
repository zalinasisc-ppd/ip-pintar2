...
EXPECTED_COLUMNS = {
    ...
    "GPMP Semasa": ["GPMP Semasa", "Semasa"],
    "GPMP Lepas": ["GPMP Lepas", "Lepas"],
}
...
def compute_ipmp(df: pd.DataFrame) -> pd.DataFrame:
    ...
    needed = ["Mata Pelajaran", "Bil. Ambil", "GPMP Semasa", "GPMP Lepas"]
    for c in needed:
        if c not in df.columns:
            raise ValueError(f"Kolum '{c}' tiada dalam data.")
    # Coerce numerics
    for c in ["Bil. Daftar", "Bil. Ambil", "GPMP Semasa", "GPMP Lepas"]:
        if c in df.columns:
            df[c] = pd.to_numeric(df[c], errors="coerce").fillna(0)

    # Calculations
    df["Beza GPMP (Lepas - Semasa)"] = df["GPMP Lepas"] - df["GPMP Semasa"]
    total_ambil = df["Bil. Ambil"].sum()
    df["Pemberat"] = np.where(total_ambil > 0, df["Bil. Ambil"] / total_ambil, 0.0)
    df["IPMP"] = df["Beza GPMP (Lepas - Semasa)"] * df["Pemberat"]
    ...
    cols = [
        ...
        "GPMP Semasa", "GPMP Lepas",
        "Beza GPMP (Lepas - Semasa)", "Pemberat", "IPMP", "Ranking"
    ]
...
st.sidebar.markdown("---")
st.sidebar.caption("Kolum diperlukan: Mata Pelajaran, Bil. Ambil, GPMP Semasa, GPMP Lepas. (Bil. Daftar opsyenal).")
...
st.subheader("Perbandingan GPMP (Lepas vs Semasa)")
long_df = df_result.melt(id_vars=["Mata Pelajaran"], value_vars=["GPMP Lepas", "GPMP Semasa"],
